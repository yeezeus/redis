...
func (c *Controller) deleteSecret(dormantDb *tapi.DormantDatabase) error {
	...
	redisList, err := c.ExtClient.Xdbs(dormantDb.Namespace).List(metav1.ListOptions{})
	if err != nil {
		return err
	}
...
func (c *Controller) ResumeDatabase(dormantDb *tapi.DormantDatabase) error {
	...
	if origin.Spec.Xdb.Init != nil {
		return errors.New("do not support InitSpec in spec.origin.redis")
	}
	...
	_, err := c.ExtClient.Xdbs(redis.Namespace).Create(redis)
	return err
}
