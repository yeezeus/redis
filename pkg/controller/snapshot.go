...
func (c *Controller) GetDatabase(snapshot *tapi.Snapshot) (runtime.Object, error) {
	redis, err := c.ExtClient.Xdbs(snapshot.Namespace).Get(snapshot.Spec.DatabaseName, metav1.GetOptions{})
	if err != nil {
		return nil, err
	}
...
func (c *Controller) GetSnapshotter(snapshot *tapi.Snapshot) (*batch.Job, error) {
	...
	redis, err := c.ExtClient.Xdbs(snapshot.Namespace).Get(databaseName, metav1.GetOptions{})
	if err != nil {
		return nil, err
	}

	// Get PersistentVolume object for Backup Util pod.
	persistentVolume, err := c.getVolumeForSnapshot(redis.Spec.Storage, jobName, snapshot.Namespace)
	if err != nil {
		return nil, err
	}
					...
					Containers: []core.Container{
						{
							...
							Image: fmt.Sprintf("%s:%s-util", docker.ImageXdb, redis.Spec.Version),
							Args: []string{
